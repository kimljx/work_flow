...
def serialize_mail_poll_state(db: Session) -> MailPollStateOut:
    ...
    protocol = runtime.mail_inbox_protocol if runtime.mail_inbox_protocol in {"imap", "pop3"} else "imap"
...
def get_scheduler_settings(_: User = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    return runtime_settings_dict(db)
