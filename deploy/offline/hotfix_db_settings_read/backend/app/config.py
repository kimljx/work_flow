...
def _read_runtime_overrides() -> dict[str, Any]:
    # 运行时业务配置已迁移到数据库。这里保留空钩子，避免旧 JSON 文件继续覆盖 PostgreSQL 中的新配置。
    return {}


def _read_database_runtime_overrides() -> dict[str, Any]:
    """读取数据库中的运行时业务配置，供仍使用 settings.xxx 的旧业务链路透明复用。"""
    try:
...
class SettingsProxy:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name not in EDITABLE_SETTING_FIELDS and name != "qax_browser_headless":
            return default
        # 旧代码仍有直接读取 settings.qax_*、settings.smtp_* 的链路，这里统一桥接到数据库配置。
        overrides = {**_read_runtime_overrides(), **_read_database_runtime_overrides()}
